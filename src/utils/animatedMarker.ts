...
L.AnimatedMarker = L.Marker.extend({
    ...
    var Icon = L.DivIcon.extend({
      ...
      rotate(deg: number) {
        if (v.before && Math.abs(v.before - deg) >= 180) {
          ...
          v.div.style.transition = "transform linear 100ms";
          // var _speed = 100 / v.options.speedX + 'ms'
          // v.div.style.transition = 'transform linear ' + _speed
        }
        
        
        v.div.style.transform =
          `translate3d(-${iconOption.iconAnchor[0]}px, -${iconOption.iconAnchor[1]}px, 0) rotate(-${deg}deg) rotateX(180deg)`;
        v.before = deg;
      },
  ...
  setSpeedX: function (_speedX: any) {
    var speed = _speedX / this.options.speedX;
    this._latLngsNew = [];
    for (var i = 0; i < this._latlngs.length; i++) {
      var _latlng: any = L.latLng(this._latlngs[i].lat, this._latlngs[i].lng);
      _latlng.duration = this._latlngs[i].duration / speed;
      _latlng.bearing = this._latlngs[i].bearing;
      this._latLngsNew.push(_latlng);
    }
    this.options.speedX = _speedX;
  ...
  getRotation: function (start: any, end: any) {
